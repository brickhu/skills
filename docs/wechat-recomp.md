...
依赖：零依赖，纯原生实现
---
回复【确认】复刻成 SolidJS + StyleX 技术栈的组件源码和使用示范
```
...
往大里说，这背后是组件消费方式的一次范式转变：

- **旧范式——选库。** 挑一个组件库装进项目，接受它的框架、样式和设计 token，跟着它的升级节奏走。组件库是"别人家的厨房"，你得搬进去做饭
- **新范式——复刻。** 行为从任何库的文档中来，样式从你的设计系统中来。组件成为你自己的代码：框架无关、样式无关、免安装、不背升级义务。组件库只是"菜谱的出处"，厨房永远是你自己的

**从"选组件库"到"复刻组件"——组件服务的不是别人的设计，而是你的设计。这是 recomp 想推动的前端新范式。**
