...
## recomp 就是这位帮厨——对着成品菜，反推菜谱

你在文档站看中一道菜，把它的页面（URL）递给帮厨——它对着这道菜反推出**菜谱**，再翻译成你家灶台的用法（headless 组件源码）：
...
1. **看不懂的菜，明说，绝不瞎编。** 你递过来的是博客、工具库、设计稿？直接拒单并说明原因。ARIA、焦点、键盘这些"味道"它不猜——要么按规范补，要么明确告诉你这块没做，宁可留白不做假
2. **不进你的厨房乱动。** 探测你的框架、CSS 方案、设计 token 时只读——不建目录、不改文件，探测结果只用在这一单上，用完即弃
3. **你家会做的菜，它不重复做——但会偷师。** Solid 有 Kobalte、Vue 有 Reka UI、Svelte 有 Bits UI——生态里已有等价实现，它拿来当参考学（人家的行为写得准），但**依然把复刻好的代码和使用指南交到你手上**，不会让你去装第三方库、把组件库的命脉交到别人手里
...
1. **点菜**——发组件库文档 URL（比如 shadcn/ui 的 Dialog 文档）
2. **看菜谱**——它先报一份"行为契约摘要"：接口、行为、每一步做法的出处（来自文档 / 按规范补全 / 参考生态实现 / 留给你拍板），你确认再看代码
